...
class SevenVGGDo(model.CNNModel):
    ...
    def define_model(self):
        ...
        model_tbd.add(Dropout(0.2))
        # block 5
        model_tbd.add(Conv2D(512, (3, 3), activation='relu', kernel_initializer='he_uniform', padding='same'))
        model_tbd.add(MaxPooling2D((2, 2)))
        model_tbd.add(Dropout(0.2))
        # block 6
        model_tbd.add(Conv2D(1024, (3, 3), activation='relu', kernel_initializer='he_uniform', padding='same'))
        model_tbd.add(MaxPooling2D((2, 2)))
        model_tbd.add(Dropout(0.2))
        # block 7
        model_tbd.add(Conv2D(2048, (3, 3), activation='relu', kernel_initializer='he_uniform', padding='same'))
        model_tbd.add(MaxPooling2D((2, 2)))
        model_tbd.add(Dropout(0.2))
